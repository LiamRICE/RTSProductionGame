## Variables
var xp:float ## The previous instruction for position (X previous)
var yd:float ## The response velocity of the interpolation (Y derivative)
var y:float ## The response position (Y)
...
func _init(frequency:float, damping:float, response:float, position_instruction:float) -> void:
	## Compute constants
...
func update(delta:float, x:float, xd:float = 0.0) -> float:
	if xd == 0.0:
		xd = (x - self.xp) / delta
		self.xp = x
