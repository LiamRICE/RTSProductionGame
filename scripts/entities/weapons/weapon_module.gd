		...
		for i in range(len(self.offsets)):
			if self.target_is_entity:
				if self.current_offset == self.offsets[i]:
					self._fire_at_entity(i, self.target_unit, self.is_hits[i])
			else:
				self._fire_at_position(i, self.target_position)
...
func _fire_at_entity(id:int, target:Entity, is_hit:bool):
	...
	if "receive_damage" in target:
		...
		if target.receive_damage(self.weapon_type.damage):
...
func _fire_at_position(id:int, target:Vector3):
